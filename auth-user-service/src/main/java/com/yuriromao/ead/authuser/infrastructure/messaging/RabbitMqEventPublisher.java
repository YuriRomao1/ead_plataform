 ...
 *
 * The adapter receives already sanitized event models and only translates them
 * to the configured RabbitMQ exchange and routing key.
 */
@Component
public class RabbitMqEventPublisher implements EventPublisher {
	...
	public RabbitMqEventPublisher(
			...
			@Value("${auth-user-service.messaging.rabbitmq.user-created-routing-key}") String userCreatedRoutingKey) {
	...
	public void publish(UserCreatedEvent event) {
		rabbitTemplate.convertAndSend(exchangeName, userCreatedRoutingKey, Objects.requireNonNull(event, "event must not be null"));
	}
