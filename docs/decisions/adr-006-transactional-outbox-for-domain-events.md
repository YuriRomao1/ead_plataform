...
A tabela `outbox_events` do `auth-user-service` deve separar metadados consultáveis do conteúdo do evento. A chave primária local é `id`; `aggregate_type` e `aggregate_id` identificam o aggregate de origem; `event_type` e `event_id` identificam o evento; `event_id` deve ser único para rastreabilidade e deduplicação. O campo `payload` deve ser `JSONB` e armazenar o envelope sanitizado do evento, incluindo `eventId`, `eventType`, `occurredAt` e o payload público. Dessa forma, o relay consegue republicar exatamente o evento de domínio sem depender de uma coluna física `occurred_at`.

O estado de publicação deve ser controlado por `status`, aceitando somente `PENDING`, `PUBLISHED` e `FAILED`. `attempts` não pode ser negativo. `next_attempt_at` deve ser obrigatório para manter a fila de tentativa explicitamente ordenável, inclusive para eventos finais que não serão mais buscados por terem status diferente de `PENDING`.
...
- criar índice para busca de eventos pendentes por `(status, next_attempt_at)`;
- criar índice para rastreio por aggregate em `(aggregate_type, aggregate_id)`;
- garantir unicidade de `event_id`;
